...
var _ = Describe("KuberhealthyCheck Controller", func() {
	Context("When reconciling a resource", func() {
		...
		kuberhealthycheck := &kuberhealthyv2.KuberhealthyCheck{}
		...
		BeforeEach(func() {
			...
			if err != nil && errors.IsNotFound(err) {
				resource := &kuberhealthyv2.KuberhealthyCheck{
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance KuberhealthyCheck")
			Expect(k8sClient.Delete(ctx, resource)).To(Succeed())
		})
		It("should successfully reconcile the resource", func() {
			By("Reconciling the created resource")
			controllerReconciler := &KuberhealthyCheckReconciler{
				Client: k8sClient,
