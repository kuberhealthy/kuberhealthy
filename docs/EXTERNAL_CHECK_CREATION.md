...
- Ensure that your check finishes within the `unix` deadline specified in the `KH_CHECK_RUN_DEADLINE` environment variable. Without respecting `KH_CHECK_RUN_DEADLINE`, your check may run into a `400` error returned from Kuberhealthy when reporting your check's status. 
