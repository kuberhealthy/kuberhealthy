...
func main() {
	...
	err = ps.Run()
	if err != nil {
		log.Errorln("Error running Pod Status check", err)
	}
}
...
func (psc *Checker) Run() error {
	...
	case <-time.After(psc.Timeout()):
		...
		errorMessage := "Failed to complete checks for " + psc.Name() + " in time!  Timeout was reached."
...
func (psc *Checker) doChecks() error {
	...
	if len(podStatus) > 0 {
		...
		for _, p := range podStatus {
			errorMessage := "Pod Status Check, found error with pod: "+p
