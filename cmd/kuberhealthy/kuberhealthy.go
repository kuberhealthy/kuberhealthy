...
// its crd status
func (k *Kuberhealthy) setCheckExecutionError(ctx context.Context, checkName string, checkNamespace string, checkErr error) error {

	// fetch the existing check to update
	checkState, err := k.getKHState(ctx, checkNamespace, checkName)
	if err != nil {
	...
	log.Debugln("Setting execution state of check", checkName, "to", checkState.Spec.OK, checkState.Spec.Errors, checkState.Spec.CurrentUUID)

	// upset the check state with the error
	checkState.Spec.OK = false
	checkState.Spec.Errors = []string{"Check execution error: " + checkErr.Error()}

	// store the check state back in the khstate crd
	err = k.storeCheckState(ctx, checkName, checkNamespace, checkState.Spec)
	if err != nil {
...
func (k *Kuberhealthy) setJobExecutionError(ctx context.Context, jobName string, jobNamespace string, exErr error) error {
	...
	khj, err := k.getJob(ctx, jobName, jobNamespace)
	if err != nil {
...
func (k *Kuberhealthy) runJob(ctx context.Context, job *khcrds.KuberhealthyJob) {
	...
	log.Infoln("Setting state of job", j.Name(), "in namespace", j.CheckNamespace(), "to", details)
...
func (k *Kuberhealthy) runCheck(ctx context.Context, c *external.Checker) {
	...
	for {
		...
		log.Infoln("Setting state of check", c.Name(), "in namespace", c.CheckNamespace(), "to", details.OK, details.Errors, details.RunDuration, details.CurrentUUID, details.KHWorkload)
...
func (k *Kuberhealthy) validateExternalRequest(ctx context.Context, selector string) (PodReportInfo, error) {
	...
	// we return an error
	whitelisted, err := k.isUUIDWhitelistedForCheck(ctx, podCheckName, podCheckNamespace, podUUID)
	if err != nil {
...
func (k *Kuberhealthy) externalCheckReportHandler(w http.ResponseWriter, r *http.Request) error {
	...
	k.externalCheckReportHandlerLog(requestID, "Setting check with name", podReport.Name, "in namespace", podReport.Namespace, "to 'OK' state:", details.OK, "uuid", details.CurrentUUID, details.KHWorkload)
